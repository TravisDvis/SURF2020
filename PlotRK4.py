...
def plotRK4Dipole(xMatrix,dt,reversal_times):
    #Initialize time
    t = 0    
    t_vector = np.linspace(0,(np.size(xMatrix,1)-1)*dt,np.size(xMatrix,1))
    #Plot dipole values
    plt.plot(t_vector,xMatrix[:][1],'b-')
    #Iterate through RK4 matrix
    for kk in range(np.size(xMatrix,1)):
        for tt in range(np.size(reversal_times,1)):
            #Condition for recognizing reversal time
            if kk == reversal_times[1][tt]: 
                #Plot vertical line when reversal occurs
                x_t = np.linspace(t,t,10)
                y_t = np.linspace(-5,5,10)
                plt.plot(x_t,y_t,'red')
        t = t + dt
    #Show plot
    plt.show()